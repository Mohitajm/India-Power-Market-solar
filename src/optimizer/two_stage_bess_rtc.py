...
def solve_stage2a_rtc(params,
    ...
    for k in range(remaining):
        ...
        prob += xc_t + y_c[k]         <= p_max,              f"C_PCS_c_{k}"
        prob += xd_t + y_d[k] + cd_t + sc_t <= p_max,       f"C2_{k}"
        # C_RTC at RT level
        ...
        prob += sc_t               <= dc_con*(1-dl[k]),    f"C3c_{k}"
        # C4: SoC dynamics
        prob += soc_lp[k+1] == (
            soc_lp[k]
...
def evaluate_actuals_rtc(params,
    ...
    return {
        ...
        "captive_committed": captive_committed,
        "rtc_notice_block":  rtc_notice_block if "rtc_notice_block" in dir() else [],
        "THRESHOLD":         THRESHOLD,
    }
